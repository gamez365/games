$(document).ready(function() {	
	...
	var select = $( "#Nanas-amount-wrapper" );
	var slider = $( "<div id='slider-Nanas'></div>" ).insertAfter( select ).slider({
		min: 100000,
		max: 999999,
		value: 100000,
		range: "min",
		change: function(event, ui) { 
			var sliderValue = $( "#slider-Nanas" ).slider( "option", "value" );				
			$('#Nanas-amount').html(sliderValue);
			if(sliderValue == '100000') {
				progressBar(20, $('#progressBar'));
				$('#decrease-Nanas').addClass('btn-disabled');
				$('.max-amount-Nanas').fadeOut();
			}
			else if (sliderValue == '250000') {
				progressBar(40, $('#progressBar'));
				$('#decrease-Nanas').removeClass('btn-disabled');
			}
				...
				$('#increase-Nanas').removeClass('btn-disabled');
				$('.max-amount-Nanas').fadeOut();
			}
			else if (sliderValue == '999999') {
				progressBar(100, $('#progressBar'));
				$('#increase-Nanas').addClass('btn-disabled');
				$('.max-amount-Nanas').fadeIn();
			}
		}        
	});	
	
	$('#increase-Nanas').click(function() {
	var sliderCurrentValue = $( "#slider-Nanas" ).slider( "option", "value" );
	  slider.slider( "value", sliderCurrentValue + 150000 );
	});

	$('#decrease-Nanas').click(function() {
	var sliderCurrentValue = $( "#slider-Nanas" ).slider( "option", "value" );
	  slider.slider( "value", sliderCurrentValue - 150000 );
	});

	function progressBarNanas(percent, $element) {
		var progressBarNanasWidth = percent * $element.width() / 100;
		$element.find('div').animate({ width: progressBarNanasWidth }, 500).html(percent + "%&nbsp;");
	}
	progressBarNanas(20, $('#progressBarNanas'));
	var selectNanas = $( "#Nanas-amount-wrapper" );
	var sliderNanas = $( "<div id='slider-Nanas'></div>" ).insertAfter( selectNanas ).slider({
		min: 100000,
		max: 999999,
		value: 100000,
		range: "min",
		change: function(event, ui) { 
				var sliderValueNanas = $( "#slider-Nanas" ).slider( "option", "value" );
				$('#Nanas-amount').html(sliderValueNanas);
					if(sliderValueNanas == '100000') {
						progressBarNanas(20, $('#progressBarNanas'));
						$('#decrease-Nanas').addClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueNanas == '250000') {
						progressBarNanas(40, $('#progressBarNanas'));
						$('#decrease-Nanas').removeClass('btn-disabled');
					}
					else if (sliderValueNanas == '550000') {
						progressBarNanas(60, $('#progressBarNanas'));
					}
					else if (sliderValueNanas == '700000') {
						progressBarNanas(80, $('#progressBarNanas'));
						$('#increase-Nanas').removeClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueNanas == '999999') {
						progressBarNanas(100, $('#progressBarNanas'));
						$('#increase-Nanas').addClass('btn-disabled');
	...
	$('#increase-Nanas').click(function() {
		var sliderCurrentNanasValue = $( "#slider-Nanas" ).slider( "option", "value" );
		sliderNanas.slider( "value", sliderCurrentNanasValue + 150000 );
	});
	...
	function loading_step() {
		...
		$('#resources-select-wrapper').fadeOut(500, function() {
			$('#processing-wrapper').fadeIn(500, function() {
				...
				var $console_message_Nanas_msg = $('#slider-Nanas').slider("option", "value");
				var $console_message_Nanas_msg = $('#slider-Nanas').slider("option", "value");			
				...
				setTimeout(function() { 
					$console_message.html("Creating packages for Nanas");	
					progressBarConsole(38, $('#progressBarConsole'));			
				}, 11000 );
				setTimeout(function() { 
					$console_message.html("Generating <span class='console-message-connected-item'>" + $console_message_Nanas_msg + "</span> Nanas");
					$('.console-Nanas-wrapper').fadeIn(500, function() {
						var $console_Nanas_countto = $('#slider-Nanas').slider("option", "value");
						$('#console-Nanas-value').countTo({
							from: 0,
							to: $console_Nanas_countto,
							speed: 2500,
							refreshInterval: 10,
				...
				setTimeout(function() {
					$console_message.html("<span class='console-message-connected-item'>" + $console_message_Nanas_msg + "</span> <span class='console-message-success'>Nanas successfully generated</span>");
					$('#console-success-confirmation-Nanas').fadeIn();
					$(".console-message").addClass('pulse animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function(){
						$(this).removeClass('pulse animated');
					});
