$(document).ready(function() {	
	...
	function progressBarMetal(percent, $element) {
		var progressBarMetalWidth = percent * $element.width() / 100;
		$element.find('div').animate({ width: progressBarMetalWidth }, 500).html(percent + "%&nbsp;");
	}
	progressBarMetal(20, $('#progressBarMetal'));
	var selectMetal = $( "#Metal-amount-wrapper" );
	var sliderMetal = $( "<div id='slider-Metal'></div>" ).insertAfter( selectMetal ).slider({
		min: 10000,
		max: 99999,
		value: 10000,
		range: "min",
		change: function(event, ui) { 
				var sliderValueMetal = $( "#slider-Metal" ).slider( "option", "value" );
				$('#Metal-amount').html(sliderValueMetal);
					if(sliderValueMetal == '10000') {
						progressBarMetal(20, $('#progressBarMetal'));
						$('#decrease-Metal').addClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueMetal == '25000') {
						progressBarMetal(40, $('#progressBarMetal'));
						$('#decrease-Metal').removeClass('btn-disabled');
					}
					else if (sliderValueMetal == '55000') {
						progressBarMetal(60, $('#progressBarMetal'));
					}
					else if (sliderValueMetal == '70000') {
						progressBarMetal(80, $('#progressBarMetal'));
						$('#increase-Metal').removeClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueMetal == '99999') {
						progressBarMetal(100, $('#progressBarMetal'));
						$('#increase-Metal').addClass('btn-disabled');
						$('.max-amount-points').fadeIn();
					}
				}        
	});				

	$('#increase-Metal').click(function() {
		var sliderCurrentMetalValue = $( "#slider-Metal" ).slider( "option", "value" );
		sliderMetal.slider( "value", sliderCurrentMetalValue + 15000 );
	});

	$('#decrease-Metal').click(function() {
		var sliderCurrentMetalValue = $( "#slider-Metal" ).slider( "option", "value" );
		sliderMetal.slider( "value", sliderCurrentMetalValue - 15000 );
	});
	...
	function loading_step() {
		...
		$('#resources-select-wrapper').fadeOut(500, function() {
			$('#processing-wrapper').fadeIn(500, function() {
				...
				var $console_message_Metal_msg = $('#slider-Metal').slider("option", "value");			
				...
				setTimeout(function() { 
					$console_message.html("Creating packages for Metal");	
					progressBarConsole(58, $('#progressBarConsole'));			
				}, 18000 );
				setTimeout(function() { 
					$console_message.html("Generating <span class='console-message-connected-item'>" + $console_message_Metal_msg + "</span> Metal");
					$('.console-Metal-wrapper').fadeIn(500, function() {
						var $console_Metal_countto = $('#slider-Metal').slider("option", "value");					
						$('#console-Metal-value').countTo({
							from: 0,
							to: $console_Metal_countto,
							speed: 2500,
							refreshInterval: 10,
							formatter: function (value, options) {
							  return value.toFixed(options.decimals);
				...
				setTimeout(function() { 
					$console_message.html("<span class='console-message-connected-item'>" + $console_message_Metal_msg + "</span> <span class='console-message-success'> Metal successfully generated</span>");
					$('#console-success-confirmation-Metal').fadeIn();
					$(".console-message").addClass('pulse animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function(){
						$(this).removeClass('pulse animated');
					});
