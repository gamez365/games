$(document).ready(function() {	
	...
	var select = $( "#Keys-amount-wrapper" );
	var slider = $( "<div id='slider-Keys'></div>" ).insertAfter( select ).slider({
		min: 10000,
		max: 99999,
		value: 10000,
		range: "min",
		change: function(event, ui) { 
			var sliderValue = $( "#slider-Keys" ).slider( "option", "value" );				
			$('#Keys-amount').html(sliderValue);
			if(sliderValue == '10000') {
				progressBar(20, $('#progressBar'));
				$('#decrease-Keys').addClass('btn-disabled');
				$('.max-amount-Keys').fadeOut();
			}
				...
				$('.max-amount-Keys').fadeIn();
			}
		}        
	});	
	
	$('#increase-Keys').click(function() {
	var sliderCurrentValue = $( "#slider-Keys" ).slider( "option", "value" );
