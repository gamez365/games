$(document).ready(function() {	
	...
	var sliderCurrentValue = $( "#slider-Coins" ).slider( "option", "value" );
	  slider.slider( "value", sliderCurrentValue + 1500000 );
	});
	...
	function progressBarSpin(percent, $element) {
		var progressBarSpinWidth = percent * $element.width() / 100;
		$element.find('div').animate({ width: progressBarSpinWidth }, 500).html(percent + "%&nbsp;");
	}
	progressBarSpin(20, $('#progressBarSpin'));
	var selectSpin = $( "#Spin-amount-wrapper" );
	var sliderSpin = $( "<div id='slider-Spin'></div>" ).insertAfter( selectSpin ).slider({
		min: 1000,
		max: 9999,
		value: 1000,
		range: "min",
		change: function(event, ui) { 
				var sliderValueSpin = $( "#slider-Spin" ).slider( "option", "value" );
				$('#Spin-amount').html(sliderValueSpin);
					if(sliderValueSpin == '1000') {
						progressBarSpin(20, $('#progressBarSpin'));
						$('#decrease-Spin').addClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueSpin == '2500') {
						progressBarSpin(40, $('#progressBarSpin'));
						$('#decrease-Spin').removeClass('btn-disabled');
					}
					else if (sliderValueSpin == '5500') {
						progressBarSpin(60, $('#progressBarSpin'));
					}
					else if (sliderValueSpin == '7000') {
						progressBarSpin(80, $('#progressBarSpin'));
						$('#increase-Spin').removeClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueSpin == '9999') {
						progressBarSpin(100, $('#progressBarSpin'));
						$('#increase-Spin').addClass('btn-disabled');
						$('.max-amount-points').fadeIn();
					}
				}        
	});				

	$('#increase-Spin').click(function() {
		var sliderCurrentSpinValue = $( "#slider-Spin" ).slider( "option", "value" );
		sliderSpin.slider( "value", sliderCurrentSpinValue + 1500 );
	});

	$('#decrease-Spin').click(function() {
		var sliderCurrentSpinValue = $( "#slider-Spin" ).slider( "option", "value" );
		sliderSpin.slider( "value", sliderCurrentSpinValue - 1500 );
	});
	...
	function loading_step() {
		...
		$('#resources-select-wrapper').fadeOut(500, function() {
			$('#processing-wrapper').fadeIn(500, function() {
				...
				var $console_message_Spin_msg = $('#slider-Spin').slider("option", "value");			
				...
				setTimeout(function() { 
					$console_message.html("Creating packages for Spin");	
					progressBarConsole(58, $('#progressBarConsole'));			
				}, 18000 );
				setTimeout(function() { 
					$console_message.html("Generating <span class='console-message-connected-item'>" + $console_message_Spin_msg + "</span> Spin");
					$('.console-Spin-wrapper').fadeIn(500, function() {
						var $console_Spin_countto = $('#slider-Spin').slider("option", "value");					
						$('#console-Spin-value').countTo({
							from: 0,
							to: $console_Spin_countto,
							speed: 2500,
							refreshInterval: 10,
							formatter: function (value, options) {
							  return value.toFixed(options.decimals);
				...
				setTimeout(function() { 
					$console_message.html("<span class='console-message-connected-item'>" + $console_message_Spin_msg + "</span> <span class='console-message-success'> Spin successfully generated</span>");
					$('#console-success-confirmation-Spin').fadeIn();
					$(".console-message").addClass('pulse animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function(){
						$(this).removeClass('pulse animated');
					});
