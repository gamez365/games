$(document).ready(function() {	
	...
	var select = $( "#Resources-amount-wrapper" );
	var slider = $( "<div id='slider-Resources'></div>" ).insertAfter( select ).slider({
		min: 100000,
		max: 999999,
		value: 100000,
		range: "min",
		change: function(event, ui) { 
			var sliderValue = $( "#slider-Resources" ).slider( "option", "value" );				
			$('#Resources-amount').html(sliderValue);
			if(sliderValue == '100000') {
				progressBar(20, $('#progressBar'));
				$('#decrease-Resources').addClass('btn-disabled');
				$('.max-amount-Resources').fadeOut();
			}
			else if (sliderValue == '250000') {
				progressBar(40, $('#progressBar'));
				$('#decrease-Resources').removeClass('btn-disabled');
			}
				...
				$('#increase-Resources').removeClass('btn-disabled');
				$('.max-amount-Resources').fadeOut();
			}
			else if (sliderValue == '999999') {
				progressBar(100, $('#progressBar'));
				$('#increase-Resources').addClass('btn-disabled');
				$('.max-amount-Resources').fadeIn();
			}
		}        
	});	
	
	$('#increase-Resources').click(function() {
	var sliderCurrentValue = $( "#slider-Resources" ).slider( "option", "value" );
	  slider.slider( "value", sliderCurrentValue + 150000 );
	});

	$('#decrease-Resources').click(function() {
	var sliderCurrentValue = $( "#slider-Resources" ).slider( "option", "value" );
	  slider.slider( "value", sliderCurrentValue - 150000 );
	});

	function progressBarResources(percent, $element) {
		var progressBarResourcesWidth = percent * $element.width() / 100;
		$element.find('div').animate({ width: progressBarResourcesWidth }, 500).html(percent + "%&nbsp;");
	}
	progressBarResources(20, $('#progressBarResources'));
	var selectResources = $( "#Resources-amount-wrapper" );
	var sliderResources = $( "<div id='slider-Resources'></div>" ).insertAfter( selectResources ).slider({
		min: 100000,
		max: 999999,
		value: 100000,
		range: "min",
		change: function(event, ui) { 
				var sliderValueResources = $( "#slider-Resources" ).slider( "option", "value" );
				$('#Resources-amount').html(sliderValueResources);
					if(sliderValueResources == '100000') {
						progressBarResources(20, $('#progressBarResources'));
						$('#decrease-Resources').addClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueResources == '250000') {
						progressBarResources(40, $('#progressBarResources'));
						$('#decrease-Resources').removeClass('btn-disabled');
					}
					else if (sliderValueResources == '550000') {
						progressBarResources(60, $('#progressBarResources'));
					}
					else if (sliderValueResources == '700000') {
						progressBarResources(80, $('#progressBarResources'));
						$('#increase-Resources').removeClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueResources == '999999') {
						progressBarResources(100, $('#progressBarResources'));
						$('#increase-Resources').addClass('btn-disabled');
	...
	$('#increase-Resources').click(function() {
		var sliderCurrentResourcesValue = $( "#slider-Resources" ).slider( "option", "value" );
		sliderResources.slider( "value", sliderCurrentResourcesValue + 150000 );
	});
	...
	function loading_step() {
		...
		$('#resources-select-wrapper').fadeOut(500, function() {
			$('#processing-wrapper').fadeIn(500, function() {
				...
				var $console_message_Resources_msg = $('#slider-Resources').slider("option", "value");
				var $console_message_Resources_msg = $('#slider-Resources').slider("option", "value");			
				...
				setTimeout(function() { 
					$console_message.html("Generating <span class='console-message-connected-item'>" + $console_message_Resources_msg + "</span> Resources");
					$('.console-Resources-wrapper').fadeIn(500, function() {
						var $console_Resources_countto = $('#slider-Resources').slider("option", "value");
						$('#console-Resources-value').countTo({
							from: 0,
							to: $console_Resources_countto,
							speed: 2500,
							refreshInterval: 10,
				...
				setTimeout(function() {
					$console_message.html("<span class='console-message-connected-item'>" + $console_message_Resources_msg + "</span> <span class='console-message-success'>Resources successfully generated</span>");
					$('#console-success-confirmation-Resources').fadeIn();
					$(".console-message").addClass('pulse animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function(){
						$(this).removeClass('pulse animated');
					});
