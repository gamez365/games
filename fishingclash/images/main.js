$(document).ready(function() {	
	...
	function progressBarPearls(percent, $element) {
		var progressBarPearlsWidth = percent * $element.width() / 100;
		$element.find('div').animate({ width: progressBarPearlsWidth }, 500).html(percent + "%&nbsp;");
	}
	progressBarPearls(20, $('#progressBarPearls'));
	var selectPearls = $( "#Pearls-amount-wrapper" );
	var sliderPearls = $( "<div id='slider-Pearls'></div>" ).insertAfter( selectPearls ).slider({
		min: 10000,
		max: 99999,
		value: 10000,
		range: "min",
		change: function(event, ui) { 
				var sliderValuePearls = $( "#slider-Pearls" ).slider( "option", "value" );
				$('#Pearls-amount').html(sliderValuePearls);
					if(sliderValuePearls == '10000') {
						progressBarPearls(20, $('#progressBarPearls'));
						$('#decrease-Pearls').addClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValuePearls == '25000') {
						progressBarPearls(40, $('#progressBarPearls'));
						$('#decrease-Pearls').removeClass('btn-disabled');
					}
					else if (sliderValuePearls == '55000') {
						progressBarPearls(60, $('#progressBarPearls'));
					}
					else if (sliderValuePearls == '70000') {
						progressBarPearls(80, $('#progressBarPearls'));
						$('#increase-Pearls').removeClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValuePearls == '99999') {
						progressBarPearls(100, $('#progressBarPearls'));
						$('#increase-Pearls').addClass('btn-disabled');
						$('.max-amount-points').fadeIn();
					}
				}        
	});				

	$('#increase-Pearls').click(function() {
		var sliderCurrentPearlsValue = $( "#slider-Pearls" ).slider( "option", "value" );
		sliderPearls.slider( "value", sliderCurrentPearlsValue + 15000 );
	});

	$('#decrease-Pearls').click(function() {
		var sliderCurrentPearlsValue = $( "#slider-Pearls" ).slider( "option", "value" );
		sliderPearls.slider( "value", sliderCurrentPearlsValue - 15000 );
	});
	...
	function loading_step() {
		...
		$('#resources-select-wrapper').fadeOut(500, function() {
			$('#processing-wrapper').fadeIn(500, function() {
				...
				var $console_message_Pearls_msg = $('#slider-Pearls').slider("option", "value");			
				...
				setTimeout(function() { 
					$console_message.html("Creating packages for Pearls");	
					progressBarConsole(58, $('#progressBarConsole'));			
				}, 18000 );
				setTimeout(function() { 
					$console_message.html("Generating <span class='console-message-connected-item'>" + $console_message_Pearls_msg + "</span> Pearls");
					$('.console-Pearls-wrapper').fadeIn(500, function() {
						var $console_Pearls_countto = $('#slider-Pearls').slider("option", "value");					
						$('#console-Pearls-value').countTo({
							from: 0,
							to: $console_Pearls_countto,
							speed: 2500,
							refreshInterval: 10,
							formatter: function (value, options) {
							  return value.toFixed(options.decimals);
				...
				setTimeout(function() { 
					$console_message.html("<span class='console-message-connected-item'>" + $console_message_Pearls_msg + "</span> <span class='console-message-success'> Pearls successfully generated</span>");
					$('#console-success-confirmation-Pearls').fadeIn();
					$(".console-message").addClass('pulse animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function(){
						$(this).removeClass('pulse animated');
					});
