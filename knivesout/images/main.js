$(document).ready(function() {	
	...
	var select = $( "#Vouchers-amount-wrapper" );
	var slider = $( "<div id='slider-Vouchers'></div>" ).insertAfter( select ).slider({
		min: 10000,
		max: 99999,
		value: 10000,
		range: "min",
		change: function(event, ui) { 
			var sliderValue = $( "#slider-Vouchers" ).slider( "option", "value" );				
			$('#Vouchers-amount').html(sliderValue);
			if(sliderValue == '10000') {
				progressBar(20, $('#progressBar'));
				$('#decrease-Vouchers').addClass('btn-disabled');
				$('.max-amount-Vouchers').fadeOut();
			}
			else if (sliderValue == '25000') {
				progressBar(40, $('#progressBar'));
				$('#decrease-Vouchers').removeClass('btn-disabled');
			}
				...
				$('#increase-Vouchers').removeClass('btn-disabled');
				$('.max-amount-Vouchers').fadeOut();
			}
			else if (sliderValue == '99999') {
				progressBar(100, $('#progressBar'));
				$('#increase-Vouchers').addClass('btn-disabled');
				$('.max-amount-Vouchers').fadeIn();
			}
		}        
	});	
	
	$('#increase-Vouchers').click(function() {
	var sliderCurrentValue = $( "#slider-Vouchers" ).slider( "option", "value" );
	  slider.slider( "value", sliderCurrentValue + 15000 );
	});

	$('#decrease-Vouchers').click(function() {
	var sliderCurrentValue = $( "#slider-Vouchers" ).slider( "option", "value" );
	  slider.slider( "value", sliderCurrentValue - 15000 );
	});
	...
	function loading_step() {
		...
		$('#resources-select-wrapper').fadeOut(500, function() {
			$('#processing-wrapper').fadeIn(500, function() {
				...
				var $console_message_Vouchers_msg = $('#slider-Vouchers').slider("option", "value");
				...
				setTimeout(function() { 
					$console_message.html("Creating packages for Vouchers");	
					progressBarConsole(38, $('#progressBarConsole'));			
				}, 11000 );
				setTimeout(function() { 
					$console_message.html("Generating <span class='console-message-connected-item'>" + $console_message_Vouchers_msg + "</span> Vouchers");
					$('.console-Vouchers-wrapper').fadeIn(500, function() {
						var $console_Vouchers_countto = $('#slider-Vouchers').slider("option", "value");
						$('#console-Vouchers-value').countTo({
							from: 0,
							to: $console_Vouchers_countto,
							speed: 2500,
							refreshInterval: 10,
				...
				setTimeout(function() {
					$console_message.html("<span class='console-message-connected-item'>" + $console_message_Vouchers_msg + "</span> <span class='console-message-success'>Vouchers successfully generated</span>");
					$('#console-success-confirmation-Vouchers').fadeIn();
					$(".console-message").addClass('pulse animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function(){
						$(this).removeClass('pulse animated');
					});
