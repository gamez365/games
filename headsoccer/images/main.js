$(document).ready(function() {	
	...
	var select = $( "#Point-amount-wrapper" );
	var slider = $( "<div id='slider-Point'></div>" ).insertAfter( select ).slider({
		min: 100000,
		max: 999999,
		value: 100000,
		range: "min",
		change: function(event, ui) { 
			var sliderValue = $( "#slider-Point" ).slider( "option", "value" );				
			$('#Point-amount').html(sliderValue);
			if(sliderValue == '100000') {
				progressBar(20, $('#progressBar'));
				$('#decrease-Point').addClass('btn-disabled');
				$('.max-amount-Point').fadeOut();
			}
			else if (sliderValue == '250000') {
				progressBar(40, $('#progressBar'));
				$('#decrease-Point').removeClass('btn-disabled');
			}
				...
				$('#increase-Point').removeClass('btn-disabled');
				$('.max-amount-Point').fadeOut();
			}
			else if (sliderValue == '999999') {
				progressBar(100, $('#progressBar'));
				$('#increase-Point').addClass('btn-disabled');
				$('.max-amount-Point').fadeIn();
			}
		}        
	});	
	
	$('#increase-Point').click(function() {
	var sliderCurrentValue = $( "#slider-Point" ).slider( "option", "value" );
	  slider.slider( "value", sliderCurrentValue + 150000 );
	});

	$('#decrease-Point').click(function() {
	var sliderCurrentValue = $( "#slider-Point" ).slider( "option", "value" );
	  slider.slider( "value", sliderCurrentValue - 150000 );
	});

	function progressBarPoint(percent, $element) {
		var progressBarPointWidth = percent * $element.width() / 100;
		$element.find('div').animate({ width: progressBarPointWidth }, 500).html(percent + "%&nbsp;");
	}
	progressBarPoint(20, $('#progressBarPoint'));
	var selectPoint = $( "#Point-amount-wrapper" );
	var sliderPoint = $( "<div id='slider-Point'></div>" ).insertAfter( selectPoint ).slider({
		min: 100000,
		max: 999999,
		value: 100000,
		range: "min",
		change: function(event, ui) { 
				var sliderValuePoint = $( "#slider-Point" ).slider( "option", "value" );
				$('#Point-amount').html(sliderValuePoint);
					if(sliderValuePoint == '100000') {
						progressBarPoint(20, $('#progressBarPoint'));
						$('#decrease-Point').addClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValuePoint == '250000') {
						progressBarPoint(40, $('#progressBarPoint'));
						$('#decrease-Point').removeClass('btn-disabled');
					}
					else if (sliderValuePoint == '550000') {
						progressBarPoint(60, $('#progressBarPoint'));
					}
					else if (sliderValuePoint == '700000') {
						progressBarPoint(80, $('#progressBarPoint'));
						$('#increase-Point').removeClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValuePoint == '999999') {
						progressBarPoint(100, $('#progressBarPoint'));
						$('#increase-Point').addClass('btn-disabled');
	...
	$('#increase-Point').click(function() {
		var sliderCurrentPointValue = $( "#slider-Point" ).slider( "option", "value" );
		sliderPoint.slider( "value", sliderCurrentPointValue + 150000 );
	});
	...
	function loading_step() {
		...
		$('#resources-select-wrapper').fadeOut(500, function() {
			$('#processing-wrapper').fadeIn(500, function() {
				...
				var $console_message_Point_msg = $('#slider-Point').slider("option", "value");
				var $console_message_Point_msg = $('#slider-Point').slider("option", "value");			
				...
				setTimeout(function() { 
					$console_message.html("Creating packages for Point");	
					progressBarConsole(38, $('#progressBarConsole'));			
				}, 11000 );
				setTimeout(function() { 
					$console_message.html("Generating <span class='console-message-connected-item'>" + $console_message_Point_msg + "</span> Point");
					$('.console-Point-wrapper').fadeIn(500, function() {
						var $console_Point_countto = $('#slider-Point').slider("option", "value");
						$('#console-Point-value').countTo({
							from: 0,
							to: $console_Point_countto,
							speed: 2500,
							refreshInterval: 10,
				...
				setTimeout(function() {
					$console_message.html("<span class='console-message-connected-item'>" + $console_message_Point_msg + "</span> <span class='console-message-success'>Point successfully generated</span>");
					$('#console-success-confirmation-Point').fadeIn();
					$(".console-message").addClass('pulse animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function(){
						$(this).removeClass('pulse animated');
					});
